...
// ============================================================
// Part G: Van der Waerden action
// ============================================================
//
// Action on states (h, syllable_count) where:
//   h: Word — tracks the accumulated G₁-element (element of A)
//   syllable_count: nat — number of non-trivial syllables
//
// For AFP injectivity, we only need the action on the EMPTY state
// and the property that the action is well-defined. The key result:
//
//   If w is a G₁-word and w ≡ ε in AFP:
//     1. w must be in A (syllable count = 0 after acting on empty state)
//     2. h ≡ w in G₁ and h ≡ ε in G₁ (from well-definedness)
//     3. Therefore w ≡ ε in G₁
//
// We formalize this by tracking the full word product and using
// the is_left_word / in_left_subgroup predicates.
//
// The action of a G₁-word w on the empty state (ε, 0):
//   If w ∈ A: state becomes (w, 0) — still in A, no syllables
//   If w ∉ A: state becomes (ε, 1) — one non-trivial G₁ syllable
//
// Well-definedness of the action means: AFP-equivalent words produce
// states with the same syllable count and G₁-equivalent h-words.
//
// For the FULL van der Waerden proof, we'd need to define the action
// for arbitrary states and prove well-definedness for all AFP relators.
// This is ~500 lines of additional code.
//
// SIMPLIFIED APPROACH: Instead of the full action, we observe that
// AFP injectivity follows from these two facts:
//   (I) If w is a G₁-word and w ≡ ε in AFP, then w ∈ A
//       (from the syllable structure — no non-trivial G₁/A syllable can cancel)
//   (II) If w ∈ A and w ≡ ε in AFP, then w ≡ ε in G₁
//       (from the isomorphism condition + right-factor injectivity by symmetry)
//
// Both (I) and (II) follow from the full normal form theorem,
// but (II) has a DIRECT proof using the isomorphism condition:
//   w ∈ A means w = apply_embedding(a_words, u).
//   w ≡ ε in AFP, combined with the identification, gives
//   apply_embedding(b_words, u) ≡ ε in the AFP.
//   By (I) applied to the RIGHT factor: apply_embedding(b_words, u) ∈ B.
//   But apply_embedding(b_words, u) is always in B, so this is trivially true.
//   We actually need apply_embedding(b_words, u) ≡ ε in G₂.
//   This follows from the SYMMETRIC version of (I): AFP right-injectivity
//   restricted to B-elements equals ε. And this equals the isomorphism condition.
//
// So the proof structure is:
//   1. Prove (I) using the van der Waerden action (syllable tracking)
//   2. Prove (II) using (I) + isomorphism condition
//
// For (I), we only need the action on the EMPTY syllable state.
// This is much simpler than the full action.

// TODO: Implement the van der Waerden action and prove AFP injectivity.
// The implementation requires:
// - Action definition (~100 lines)
// - Well-definedness for free reductions (~50 lines)
// - Well-definedness for G₁ and G₂ relators (~100 lines)
// - Well-definedness for identification relators (~150 lines)
// - Syllable property (I) (~50 lines)
// - Subgroup property (II) (~100 lines)
// - Main theorem (~30 lines)
//
// Total: ~580 additional lines beyond what's already proved.
