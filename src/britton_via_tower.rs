...
// --- X.17: Action chain — G₂ one-shot creates right syllable ---

/// Core textbook argument: decompose translate(w) at the rightmost Gen-Inv pair,
/// show the G₂ one-shot creates a right syllable from (ε, []).
/// The suffix is G₁ (strictly below max level), so right_count = 0 before G₂.
/// G₂ one-shot with base ∉ B prepends right syllable → right_count = 1.
/// G₁ prefix preserves right_count ≥ 1.
/// (For multi-visit: additional inter-visit argument needed for prefix.)
proof fn lemma_action_chain(
    ...
    requires
        ...
        equiv_in_presentation(tower_presentation(data, pl),
            translate_word_at(data, w, bl as int), empty_word()),
    ensures false,
{
    use crate::normal_form_afp_textbook::*;
    let ng = data.base.num_generators;
    ...
    // prefix_level + 1 = max_lev
    let prefix_word = w.subrange(0, pair_i);
    let gen_word: Word = Seq::new(1, |_j: int| w[pair_i]);
    assert(w.subrange(0, pair_i + 1) =~= concat(prefix_word, gen_word)) by {
        assert forall|k: int| 0 <= k < w.subrange(0, pair_i + 1).len()
            implies w.subrange(0, pair_i + 1)[k] == concat(prefix_word, gen_word)[k]
        by { if k < pair_i {} else {} }
    }
    lemma_net_level_concat(data, prefix_word, gen_word);
    lemma_net_level_single(data, w[pair_i]);
    assert(prefix_level + 1 == max_lev);
    ...
    lemma_translate_concat(data, w_prefix, concat(gen_base_inv, w_suffix), bl as int);
    lemma_translate_concat(data, gen_base_inv, w_suffix, bl as int + prefix_level);
    ...
    // Step 1: suffix is G₁, preserves right_count = 0
    // Suffix levels bounded: all shifted levels after pair_j+1 are ≤ junc
    // So tw_suffix is word_valid for tower(junc) = afp.p1
    // Need: the net_level at pair_j+1 in w is prefix_level (since gen_base_inv has net 0)
    // net_level(w[0..pair_j+1]) = prefix_level + 0 = prefix_level
    // Shifted level at pair_j+1: bl + prefix_level = junc
    assert(net_level(data, w.subrange(0, pair_j + 1)) == prefix_level) by {
        lemma_suffix_net_level(data, w, pair_i, pair_j + 1 - pair_i);
        assert(w.subrange(pair_i, pair_j + 1) =~= concat(Seq::new(1, |_j: int| w[pair_i]),
            concat(base_word, Seq::new(1, |_j: int| w[pair_j])))) by {
            assert forall|i: int| 0 <= i < (pair_j + 1 - pair_i)
                implies w.subrange(pair_i, pair_j + 1)[i] == concat(Seq::new(1, |_j: int| w[pair_i]),
                    concat(base_word, Seq::new(1, |_j: int| w[pair_j])))[i]
            by {
                if i == 0 {} else if i < pair_j - pair_i { assert(w[pair_i + i] == base_word[i - 1]); }
                else {}
            }
        }
        lemma_net_level_concat(data, Seq::new(1, |_j: int| w[pair_i]),
            concat(base_word, Seq::new(1, |_j: int| w[pair_j])));
        lemma_net_level_single(data, w[pair_i]);
        lemma_net_level_concat(data, base_word, Seq::new(1, |_j: int| w[pair_j]));
        lemma_net_level_single(data, w[pair_j]);
        lemma_net_level_base_word(data, base_word);
    }

    // Use suffix_levels_bounded to show suffix is G₁
    lemma_suffix_levels_bounded(data, w, pair_j + 1, bl as int, pl);

    // tw_suffix valid for tower(junc) = afp.p1
    ...
    // Need: translate(w_suffix, bl + prefix_level) valid for tower(junc)
    // From suffix_levels_bounded: all shifted levels in [0, junc]
    // Use lemma_translate_word_valid_for_level with m = junc
    ...
    by {
        if 0 <= k && k <= w_suffix.len() {
            // Connect w_suffix.subrange(0,k) to w.subrange(pair_j+1, pair_j+1+k)
    ...
    // G₁ suffix preserves right_count = 0
    lemma_act_g1_word_preserves_right_count(afp, tw_suffix, e_h, e_s);
    let (h_suf, syls_suf) = act_word(afp, tw_suffix, e_h, e_s);
    assert(right_syllable_count(syls_suf) == 0);

    // Step 2: G₂ one-shot creates right syllable
    // tw_g2 = shift(base_word, pl*ng). Apply g2_one_shot.
    lemma_act_word_eq_g2_one_shot(afp, base_word, h_suf, syls_suf);
    // act(tw_g2, h_suf, syls_suf) = g2_one_shot(concat(base_word, embed_b(h_suf)), syls_suf)
    let product = concat(base_word, apply_embedding(b_words(afp), h_suf));
    let rep = b_rcoset_rep(afp, product);

    // base_word ∉ B (from no pinch on Gen-Inv pair)
    // embed_b(h_suf) ∈ B
    // concat(base_word, embed_b(h_suf)) ∉ B (by right-cancel)
    // So rep ≠ ε → g2_one_shot prepends right syllable

    // TODO: connect "base_word ∉ B in HNN" to "base_word ∉ B at AFP junction"
    // and show rep ≠ ε → right_count ≥ 1.
    // This requires matching the HNN B-subgroup (<b_i>) with the AFP B-subgroup
    // (b_words(afp) = <b_i> since afp = tower_afp_data at the right junction).
    // Then use lemma_not_in_subgroup_concat_embed_b.

    assert(false); // Remaining: base ∉ B connection + one-shot analysis + prefix
}
