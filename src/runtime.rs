...
pub fn apply_hom_exec(
    ...
    requires
        forall|k: int| 0 <= k < w@.len() ==> match w@[k] {
            RuntimeSymbol::Gen(i) => (i as int) < h.generator_images@.len(),
            RuntimeSymbol::Inv(i) => (i as int) < h.generator_images@.len(),
        },
    ensures
            ...
            apply_hom(runtime_hom_view(h), runtime_word_view(w@)),
{
    ...
    while i < w.len()
        invariant
            0 <= i <= w.len(),
            forall|k: int| 0 <= k < w@.len() ==> match w@[k] {
                RuntimeSymbol::Gen(idx) => (idx as int) < h.generator_images@.len(),
                RuntimeSymbol::Inv(idx) => (idx as int) < h.generator_images@.len(),
            },
            hv@ == runtime_hom_view(h),
            runtime_word_view(result@) =~=
                apply_hom(hv@, runtime_word_view(w@.subrange(0, i as int))),
        decreases w.len() - i,
    {
        ...
        proof {
            ...
            // apply_hom(h, sub_i1) = apply_hom(h, sub_i + singleton)
            //                      =~= concat(apply_hom(h, sub_i), apply_hom(h, singleton))
            crate::homomorphism::lemma_hom_respects_concat(hv@, sub_i, singleton);

            // apply_hom(h, singleton) =~= apply_hom_symbol(h, w[i])
            assert(singleton.first() == wv[i as int]);
            assert(singleton.drop_first() =~= empty_word());
            assert(apply_hom(hv@, singleton.drop_first()) =~= empty_word());
            assert(apply_hom(hv@, singleton) =~=
                concat(apply_hom_symbol(hv@, wv[i as int]), empty_word()));
            assert(concat(apply_hom_symbol(hv@, wv[i as int]), empty_word()) =~=
                apply_hom_symbol(hv@, wv[i as int]));
        }

        // Append sym_image to result
        let mut j: usize = 0;
        while j < sym_image.len()
            invariant
                0 <= j <= sym_image.len(),
                0 <= i < w.len(),
                hv@ == runtime_hom_view(h),
                runtime_word_view(sym_image@) =~=
                    apply_hom_symbol(hv@, runtime_symbol_view(w@[i as int])),
                // result so far = old_result + sym_image[0..j]
                runtime_word_view(result@) =~=
                    apply_hom(hv@, runtime_word_view(w@.subrange(0, i as int)))
                    + runtime_word_view(sym_image@.subrange(0, j as int)),
            decreases sym_image.len() - j,
        {
            result.push(sym_image[j]);
            j = j + 1;
        }

        proof {
            assert(sym_image@.subrange(0, sym_image@.len() as int) =~= sym_image@);
        }
